...
parser.add_argument('--run-eventsub', help='Run the eventsub client on this twitch client', action='store_true',
                    default=False)
...
parser.add_argument('--views', help='Load plugins into the bot', nargs='*', default=[])  # TODO: IMPL
parser.add_argument('--config', help='Configuration file', default=None)
parser.add_argument('--use-sigint', help='Use twitch.cli\'s built in signal handler for catching shutdown requests',
...
class CLI:
    def __init__(self):
        ...
        if self.args.run_eventsub or hasattr(self._config, 'eventsub'):
            self._client.es = EventSubClient(self._client,
                                             EventSubConfig(self._config.eventsub) if hasattr(self._config,
                                                                                              'eventsub') else EventSubConfig())

        if self.args.run_irc or hasattr(self._config, 'irc'):
            self._client.irc = IRCClient(self._client,
                                         IRCConfig(self._config.irc) if hasattr(self._config, 'irc') else IRCConfig())
